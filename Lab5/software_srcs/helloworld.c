...
int main()
{
	...
	int Index, Green, Blue, Red, line_odd_even, pixel_odd_even;
	u8 *TxBufferPtr;
	u8 *RxBufferPtr;
    	...
    	for(j=0; j < N; j++){
	    	if(line_odd_even == 0){ //grammes 0,2,...(green and blue)
	    		if(pixel_odd_even == 0){ //green
		    		Green = TxBufferPtr[i*N+j];
		    		Blue = (TxBufferPtr[i*N+j-1]+TxBufferPtr[i*N+j+1])/2;
		    		Red = (TxBufferPtr[(i-1)*N]+TxBufferPtr[(i+1)*N])/2;
		    	}
		    	else{	//blue
		    		Green = (TxBufferPtr[i*N+j-1]+TxBufferPtr[i*N+j+1]+TxBufferPtr[(i-1)*N]+TxBufferPtr[(i+1)*N])/4;
		    		Blue = TxBufferPtr[i*N+j];
		    		Red = (TxBufferPtr[(i-1)*N+j-1]+TxBufferPtr[(i-1)*N+j+1]+TxBufferPtr[(i+1)*N+j-1]+TxBufferPtr[(i+1)*N+j+1])/4;
		    	}
	    	}
	    	else{	//grammes 1,3,...(green and red)
	    		if(pixel_odd_even == 0){ //red
		    		Green = (TxBufferPtr[i*N+j-1]+TxBufferPtr[i*N+j+1]+TxBufferPtr[(i-1)*N]+TxBufferPtr[(i+1)*N])/4;
		    		Blue = (TxBufferPtr[(i-1)*N+j-1]+TxBufferPtr[(i-1)*N+j+1]+TxBufferPtr[(i+1)*N+j-1]+TxBufferPtr[(i+1)*N+j+1])/4;
		    		Red = TxBufferPtr[i*N+j];
		    	} 
		    	else{ //green
		    		Green = TxBufferPtr[i*N+j];
		    		Blue = (TxBufferPtr[(i-1)*N]+TxBufferPtr[(i+1)*N])/2;
		    		Red = (TxBufferPtr[i*N+j-1]+TxBufferPtr[i*N+j+1])/2;
		    	}
	    	}
